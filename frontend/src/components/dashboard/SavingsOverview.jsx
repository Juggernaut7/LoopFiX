  ...
  const fetchDeFiData = async () => {
    try {
      ...
      if (connectionStatus.isConnected) {
        ...
        const balanceResult = await stacksService.getWalletBalance();
        if (balanceResult.success) {
          setWalletBalance(balanceResult.balance);
        }

        // Fetch yield rates
        const yieldResult = await stacksService.getYieldRates();
        if (yieldResult.success) {
          setYieldRates(yieldResult.yieldRates);
        }

        // Fetch portfolio data
        const portfolioResult = await stacksService.getPortfolio();
  ...
  return (
    <div className="glass-card p-6">
      <div className="flex items-center justify-between mb-4">
        ...
        {stacksService.getConnectionStatus().isConnected && (
      ...
      <div className="space-y-4">
        {/* Wallet Balance (DeFi) */}
        {stacksService.getConnectionStatus().isConnected && walletBalance && (
