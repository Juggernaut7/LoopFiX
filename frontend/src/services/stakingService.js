...
class StakingService {
  ...
  async stakeSTX(poolId, amount, lockPeriod) {
    try {
      ...
      // Convert to microSTX and calculate lock period in blocks
      const amountMicroSTX = Math.floor(amount * 1000000);
      const lockPeriodBlocks = Math.floor(lockPeriod * 24 * 60 * 60 / 10); // Approximate blocks per day

      console.log('🎯 Staking STX:', {
        poolId,
      ...
      // Call smart contract
      const txResult = await contractService.stakeSTX(poolId, amountMicroSTX, lockPeriodBlocks);
      ...
      console.error('Error staking STX:', error);
      return {
        success: false,
        error: error.message
      };
  ...
  async unstakeSTX(stakeId) {
    try {
      ...
      console.log('🎯 Unstaking STX:', { stakeId, walletAddress });

      // Call smart contract
      const txResult = await contractService.unstakeSTX(stakeId);
      ...
      console.error('Error unstaking STX:', error);
      return {
        success: false,
        error: error.message
      };
  ...
  async claimRewards(stakeId) {
    try {
      ...
      // Call smart contract
      const txResult = await contractService.claimRewards(stakeId);

      if (!txResult.txId) {
        throw new Error('Reward claiming transaction failed');
      }

      // Update backend
      const response = await api.put(`/staking/stakes/${stakeId}/claim`, {
        claimTxId: txResult.txId,
