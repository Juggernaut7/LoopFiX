  ...
  async getMarketAnalysis() {
    try {
      const response = await axios.post(
        `${this.apiUrl}/cardiffnlp/twitter-roberta-base-sentiment-latest`,
        {
          inputs: "Bitcoin DeFi market analysis: Current trends show increased adoption of Bitcoin Layer 2 solutions, with Stacks leading in smart contract development. Yield farming opportunities are expanding with new protocols offering competitive APY rates."
        },
        { headers: this.headers }
      );
  ...
  parseDeFiResponse(aiResponse) {
    ...
    if (Array.isArray(aiResponse) && aiResponse.length > 0) {
      ...
      if (response.generated_text) {
        ...
        if (text.includes('STX staking')) {
          recommendations.push({
            type: 'staking',
            asset: 'STX',
            apy: '8.5%',
            risk: 'low',
            description: 'Stake STX for stable returns with low risk'
          });
        }
      ...
      recommendations: [
        {
          ...
          apy: '8.5%',
          risk: 'low',
          description: 'Start with STX staking for stable returns'
        },
        {
          type: 'yield_farming',
          asset: 'sBTC',
  ...
  getFallbackMarketAnalysis() {
    return {
      ...
      opportunities: [
        {
          type: 'staking',
          description: 'STX staking opportunities with stable returns',
          potential: 'high'
        }
      ]
    };
