...
const stacksService = require('./stacks.service');
...
class DeFiAIService {
  ...
  async generateDeFiRecommendations(userProfile, portfolio, marketData, yieldRates) {
    try {
      ...
      const optimalStrategy = await stacksService.calculateOptimalStrategy(userProfile, riskTolerance);
      ...
      if (portfolio.balance > 1000000) { // More than 1 STX
        recommendations.push({
          ...
          description: `You have ${(portfolio.balance / 1000000).toFixed(2)} STX available for staking`,
          apy: yieldRates.stx_staking.apy,
          risk: yieldRates.stx_staking.risk,
          potential_earnings: (portfolio.balance * yieldRates.stx_staking.apy / 100) / 1000000,
          action: 'stake_stx'
        });
      }
  ...
  calculateAdditionalEarnings(vaults, yieldRates) {
    ...
    return (totalVaultValue * additionalYield / 100) / 1000000; // Convert to STX
  }
  ...
  calculateGroupBenefit(groups, yieldRates) {
    ...
    return (totalGroupValue * additionalYield / 100) / 1000000; // Convert to STX
  }
  ...
  calculateTotalPotentialEarnings(portfolio, yieldRates) {
    ...
    return (totalValue * avgYield / 100) / 1000000; // Convert to STX
  }
  ...
  summarizePortfolio(portfolio) {
    ...
    return {
      ...
      total_value_usd: (totalValue / 1000000) * 2.45, // Assuming STX price
  ...
  analyzeMarketConditions(marketData) {
    return {
      stx_price: marketData.stx_price,
      ...
      recommended_actions: marketData.network_health > 95 ? 
        ['Consider increasing DeFi exposure', 'Stake more STX'] : 
  ...
  generateRecommendedActions(userProfile, defiRecommendations) {
    ...
    if (defiRecommendations.recommendations.length > 0) {
      ...
      actions.push('Consider staking your STX');
    }
  ...
  calculateMonthlyEarnings(portfolio, yieldRates) {
    ...
    const avgYield = (yieldRates.stx_staking.apy + yieldRates.btc_yield.apy + yieldRates.defi_pools.apy) / 3;
    return (totalValue * avgYield / 100 / 12) / 1000000; // Monthly in STX
  }
  ...
  calculateYearlyEarnings(portfolio, yieldRates) {
    ...
    const avgYield = (yieldRates.stx_staking.apy + yieldRates.btc_yield.apy + yieldRates.defi_pools.apy) / 3;
    return (totalValue * avgYield / 100) / 1000000; // Yearly in STX
  }
  ...
  async getMarketAnalysis() {
    try {
      ...
      const analysis = {
        ...
        market_trends: {
          stx_trend: marketData.marketData.stx_price > 2.0 ? 'bullish' : 'bearish',
