...
const getDeFiDashboard = async (req, res, next) => {
  try {
    ...
    // Fetch real on-chain balance from Stacks blockchain
    try {
      const onChainBalance = await stacksService.getWalletBalance(walletAddress);
      wallet.onChainBalance = onChainBalance.available;
      wallet.balance = onChainBalance.available; // Update local balance
      wallet.lastSynced = new Date();
      await wallet.save();
    } catch (error) {
      console.log('⚠️  Could not fetch on-chain balance:', error.message);
      // Continue with local balance if Stacks API fails
    }
    ...
    // Get recent activity (last 10 transactions) - combine off-chain and on-chain
    const recentActivity = await getRecentActivity(walletAddress);
    
    // Try to fetch on-chain transactions
    try {
      const onChainTxs = await stacksService.getWalletTransactions(walletAddress, 10);
      const onChainActivity = onChainTxs.map(tx => ({
        id: tx.txId,
        type: 'blockchain_transaction',
        title: tx.type === 'contract_call' ? 'Smart Contract Interaction' : 'STX Transfer',
        description: `${tx.amount} STX`,
        amount: tx.amount,
      ...
      recentActivity.push(...onChainActivity);
      recentActivity.sort((a, b) => new Date(b.date) - new Date(a.date));
    } catch (error) {
      console.log('⚠️  Could not fetch on-chain transactions:', error.message);
    }
...
async function getRecentActivity(walletAddress, limit = 10) {
  try {
    ...
    const activity = recentVaults.map(vault => ({
      ...
      description: `Started saving towards ${vault.name} with target of ${vault.targetAmount} STX`,
